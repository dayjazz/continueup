...
public class StorageServiceImpl implements StorageService {
    ...
    public void deleteAll() {
        System.out.println("开发初始化清理数据，start");
        ...
        System.out.println("开发初始化清理数据，end");
    }
    ...
    private boolean checkAndSetUploadProgress(MultipartFileParam param, String uploadDirPath) throws IOException {
        ...
        if (isComplete == Byte.MAX_VALUE) {
            redisTemplate.opsForHash().put(Constants.FILE_UPLOAD_STATUS, param.getMd5(), "true");
            redisTemplate.opsForValue().set(Constants.FILE_MD5_KEY + param.getMd5(), uploadDirPath + "/" + fileName);
            return true;
        } else {
            if (!redisTemplate.opsForHash().hasKey(Constants.FILE_UPLOAD_STATUS, param.getMd5())) {
                redisTemplate.opsForHash().put(Constants.FILE_UPLOAD_STATUS, param.getMd5(), "false");
            }
            if (redisTemplate.hasKey(Constants.FILE_MD5_KEY + param.getMd5())) {
                redisTemplate.opsForValue().set(Constants.FILE_MD5_KEY + param.getMd5(), uploadDirPath + "/" + fileName + ".conf");
            }
            return false;
        }
    }
